...
mod tests {
    ...
    fn user(user: &str) -> AccountId {
        (user.to_owned() + ".sub.testnet").parse().unwrap()
    }
    ...
    fn setup_contract() -> Contract {
        ...
        let near_entry_fees: Vec<U128> = vec![
            ONE_NEAR.into(),
            (3 * ONE_NEAR).into(),
            (5 * ONE_NEAR).into(),
        ];
    ...
    fn contract_context() -> (Contract, VMContextBuilder) {
        ...
        assert_eq!(
            get_owner(&mut contract),
            owner(),
            "owner.near must be a contract owner"
        );
        (contract, get_context(owner()))
    }

    fn owner_env(context: &mut VMContextBuilder) {
        testing_env!(context
    ...
    fn change_subs(contract: &mut Contract, context: &mut VMContextBuilder) {
        owner_env(context);
        contract.change_accepted_subs("sub.testnet".into());
    }
    ...
    fn enter_lottery(
        ...
        let prev_lotteries_num = contract.get_lotteries_num();
        let lottery_id =
            contract.draw_near_enter(lottery_type, lottery_num_participants, referrer_id);
        ...
        if is_first {
            assert!(
                contract.get_lotteries_num() == prev_lotteries_num + 1,
                "contract expected added new lottery instance"
            );
            assert_eq!(entries_num, 1);
            assert_eq!(current_pool, entry_fee.0);
        }
        ...
        assert_eq!(lottery.entry_fee.0, entry_fee.0);
        assert_eq!(
            lottery.required_pool.0,
            entry_fee.0 * lottery_num_participants as u128
        );
    ...
    fn test_modify_lottery_config() {
        ...
        assert!(contract
            ...
            .iter()
            .all(|(_, nums_vector)| { nums_vector.contains(&25) }));
        ...
        assert!(!contract
            ...
            .num_participants_required
            .iter()
            .any(|(_, nums_vector)| { nums_vector.contains(&25) }));
    }
    ...
    fn test_finished_lottery() {
        ...
        assert_eq!(
            params.fees_collected[0],
            (near(), U128(keeped_fees)),
