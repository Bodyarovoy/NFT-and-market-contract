use near_sdk::{json_types::U64, require};

use crate::{
    views::{LotteryResult, SimpleLotteryResult},
    *,
};
...
impl Lottery {
    ...
    pub fn update(&mut self) -> LotteryStatus {
        match self {
            Lottery::SimpleLottery(lottery) => lottery.update(),
...
impl Contract {
    pub(crate) fn check_accepted_subs(&self, account_id: &AccountId) {
        ...
        let accepted_sub = accepted_subs.split('.').collect::<Vec<_>>();
        let stringified_account = account_id.to_string();
        let splitted_acc = stringified_account.split('.').collect::<Vec<_>>();
    ...
    pub(crate) fn internal_get_lottery_by_parameters(
        ...
        self.lotteries.values().find(|lottery| {
    ...
    pub(crate) fn internal_get_lottery(&self, lottery_id: LotteryId) -> Option<Lottery> {
        self.lotteries.get(&lottery_id).map(|mut lottery| {
    ...
    pub fn draw_enter(
        ...
        let lottery = match self.internal_get_lottery_by_parameters(
            &lottery_token_id,
            num_participants,
            entry_fee,
        ) {
            Some(lottery) => lottery,
            None => self.add_new_lottery(
            ...
            Lottery::SimpleLottery(mut simple_lottery) => {
                ...
                match lottery_status {
                    ...
                    LotteryStatus::Finished => {
                        let lottery_result =
                            self.distribute(Lottery::SimpleLottery(simple_lottery));
                        log!("{:#?}", lottery_result);
                        self.lotteries.remove(&lottery_id);
                    }
                    // user just created entry for that lottery
                    LotteryStatus::Active => self
                        .internal_set_lottery(&lottery_id, Lottery::SimpleLottery(simple_lottery)),
                }
                lottery_id
            }
    ...
    pub fn distribute(&mut self, lottery: Lottery) -> LotteryResult {
        match lottery {
            Lottery::SimpleLottery(lottery) => {
                ...
                assert!(
                    reward > contract_fees,
                    "Reward cannot be less than contract fees"
                );
                ...
                if lottery.refferal_transfered > 0 {
                    assert!(
                        ...
                        "Refferal's reward cannot be greater than contract fees"
                    );
                    contract_fees -= lottery.refferal_transfered;
                }
                ...
                LotteryResult::SimpleLotteryResult(SimpleLotteryResult {
    ...
    pub fn add_new_lottery(
        ...
        let lottery = match lottery_type {
            LotteryType::SimpleLottery => Lottery::SimpleLottery(SimpleLottery::new(
                lottery_id,
                lottery_token_id,
                num_participants,
                entry_fee,
